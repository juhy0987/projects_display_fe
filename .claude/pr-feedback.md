...
본 명세는 저장소 차원에서 **상시 운영되는 cron** 으로 등록한다. PR-specific 인 [`loop.md`](loop.md) 와 달리, 저장소당 하나만 살아 있으면 충분하다.

```
CronCreate({
  schedule: "*/3 * * * *",
  prompt: "@.claude/pr-feedback.md 절차에 따라 신규 PR 을 감지하고 자동 리뷰 피드백을 남겨줘.",
  recurring: true
})
```

- 운영 시작 시 1회 등록 — `CronList` 로 활성 cron 에 본 prompt 가 이미 있는지 확인 후 중복 등록 회피
- 자동 종료 조건: idle (신규 PR 없음) 20회 연속 → `CronDelete` 로 자체 정리 (아래 §3)
- 종료 후 신규 PR 감시가 필요해지면 동일 형식으로 재등록
